...
void main()
{
	...
	vec4 ambient = u_Light.ambient * texture(u_Material.Texture_Diffuse0, v_TexCoord);
	...
	vec4 vectorToLight = normalize(-u_Light.lightDir);
	...
	vec4 diffuse = u_Light.diffuse * (diffStrength * texture(u_Material.Texture_Diffuse0, v_TexCoord));
	...
	vec4 specular = u_Light.specular * (spec * texture(u_Material.Texture_Specular0, v_TexCoord));
